...
import Yes3Sensitive from './Yes3Sensitive';
import No3Sensitive from './No3Sensitive';
    ...
    return (
      <>
        <div className="flex flex-col items-center py-4 dark:text-gray-300">
          <p className="text-center">Is there any swelling or sensitive between the toes or on the pads?</p>
          ...
          <p className="text-sm pt-2 text-center">Selected: {selectedButton}</p>
            <div className="flex justify-center py-4 pb-4">
              {showElement &&
                (selectedButton === "Yes" ? (
                  <Yes3Sensitive selected={selectedButton} />
                ) : (
                  <No3Sensitive selected={selectedButton} />
                ))}
            </div>
            </>
          )}
