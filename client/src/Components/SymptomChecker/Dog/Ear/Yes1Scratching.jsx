...
import Yes2Red from './Yes2Red';
import No2Red from './No2Red';

function Yes1Scratching() {
    ...
    return (
      <>
        <div className="flex flex-col items-center py-4 dark:text-gray-300">
          <p className="text-center">Is there visible redness or swelling inside the ear?</p>
          ...
          <p className="text-sm pt-2 text-center">Selected: {selectedButton}</p>
            <div className="flex justify-center py-4 pb-4">
              {showElement &&
                (selectedButton === "Yes" ? (
                  <Yes2Red selected={selectedButton} />
                ) : (
                  <No2Red selected={selectedButton} />
                ))}
            </div>
            </>
          )}
