...
import Yes3Odor from './Yes3Odor';
import No3Odor from './No3Odor';

function No1Scratching() {
    ...
    return (
      <>
        <div className="flex flex-col items-center py-4 dark:text-gray-300">
          <p className="text-center">Is there odor or discharge from the ear?</p>
          ...
          <p className="text-sm pt-2 text-center">Selected: {selectedButton}</p>
            <div className="flex justify-center py-4 pb-4">
              {showElement &&
                (selectedButton === "Yes" ? (
                  <Yes3Odor selected={selectedButton} />
                ) : (
                  <No3Odor selected={selectedButton} />
                ))}
            </div>
            </>
          )}
