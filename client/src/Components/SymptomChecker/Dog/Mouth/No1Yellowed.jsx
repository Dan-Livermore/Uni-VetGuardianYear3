...
import Yes3Eating from './Yes3Eating';
import No3Eating from './No3Eating';

function No1Yellowed() {
    ...
    return (
      <>
        <div className="flex flex-col items-center py-4 dark:text-gray-300">
          <p className="text-center">Does the dog seem to have pain when eating?</p>
          ...
          <p className="text-sm pt-2 text-center">Selected: {selectedButton}</p>
            <div className="flex justify-center py-4 pb-4">
              {showElement &&
                (selectedButton === "Yes" ? (
                  <Yes3Eating selected={selectedButton} />
                ) : (
                  <No3Eating selected={selectedButton} />
                ))}
            </div>
            </>
          )}
