...
import Yes2Tight from './Yes2Tight';
import No2Tight from './No2Tight';

function Yes1Swollen() {
    ...
    return (
      <>
        <div className="flex flex-col items-center py-4 dark:text-gray-300">
          <p className="text-center">Is the belly significantly swollen and tight?</p>
          ...
          <p className="text-sm pt-2 text-center">Selected: {selectedButton}</p>
            <div className="flex justify-center py-4 pb-4">
              {showElement &&
                (selectedButton === "Yes" ? (
                  <Yes2Tight selected={selectedButton} />
                ) : (
                  <No2Tight selected={selectedButton} />
                ))}
            </div>
            </>
          )}
