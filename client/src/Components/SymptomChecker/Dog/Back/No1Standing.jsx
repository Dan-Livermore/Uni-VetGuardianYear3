...
import Yes4Touch from './Yes4Touch';
import No4Touch from './No4Touch';

function  No1Standing() {
    ...
    return (
      <>
        <div className="flex flex-col items-center py-4 dark:text-gray-300">
          <p className="text-center">Is your pet in visible pain when touched?</p>
          ...
          <p className="text-sm pt-2 text-center">Selected: {selectedButton}</p>
            <div className="flex justify-center py-4 pb-4">
              {showElement &&
                (selectedButton === "Yes" ? (
                  <Yes4Touch selected={selectedButton} />
                ) : (
                  <No4Touch selected={selectedButton} />
                ))}
            </div>
            </>
          )}
